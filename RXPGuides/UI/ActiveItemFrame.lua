    ...
    for _, item in ipairs(itemList) do
        ...
        if not btn then
            ...
            if btn.RegisterForClicks then--and addon.game == "RETAIL" then
                -- Firing on both edges invokes toggle-style spells such as
                -- Attack twice. Match the keybinding's runOnUp behavior.
        ...
        if btn.itemId ~= item.id and btn.cooldown then
            -- 3.3.5a Cooldown frames have no :Clear() (added in 6.0); zeroing the
            -- cooldown clears the swipe. A nil :Clear here was throwing inside
            -- SetStep and silently breaking all guide-step navigation.
            if btn.cooldown.Clear then
                btn.cooldown:Clear()
            elseif btn.cooldown.SetCooldown then
